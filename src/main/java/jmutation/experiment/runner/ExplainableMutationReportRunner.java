...
public class ExplainableMutationReportRunner extends ReportRunner {
    ...
    public ExplainableMutationReportRunner(String[] defects4jProjects, int[] defects4jBugIds, ExplainableMutator mutator, Path pathToCloneDefects4J) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int[] bugs = {26};
        ExplainableMutationClient explainableMutationClient = new ExplainableMutationClient(new Client("localhost", 8080));
        Path path = Path.of("C:", "Users", "bchenghi", "Desktop", "mutation-experiment"); // Change based on system
        ExplainableMutationReportRunner experiment = new ExplainableMutationReportRunner(projects, bugs, new ExplainableMutator(explainableMutationClient), path);
        experiment.run();
    }
    ...
    public void run() throws Exception {
        ...
        for (int i = 0; i < projectNames.length; i++) {
            ...
            for (int j = 1; j <= totalBugs; j++) {
                ...
                // Compile
                Defects4jProject project = new Defects4jProject(projName, pathToCloneDefects4J.toFile(), null, null, null, null, null);
                Defects4jExecutor workingExecutor = new Defects4jExecutor(project);
                workingExecutor.checkout(projName, versionStr, pathToCloneDefects4J.toString());
                ...
                for (MutationCommand command : commands) {
                    ...
                    if (!compilationSuccess(compilationResult)) {
                        ExplainableTrial trial = new ExplainableTrial(projName,
                                versionStr, explainableMutationCommand.getOriginalMethod(), "",
                                explainableMutationCommand.getOriginalComment(), "", "COMPILATION_FAILED",
                                new String[]{}, 0, command.toString());
                        report.record(trial);
                        continue;
                    }
                    buggyExecutor.test();
                    ...
                    try {
                        ...
                        totalTests = project.getAllTests().length;
                    } catch (IOException e) {
                        e.printStackTrace();
                        continue;
                    }
                    ExplainableTrial trial = new ExplainableTrial(projName,
                            versionStr, explainableMutationCommand.getOriginalMethod(), explainableMutationCommand.getMutatedMethod(),
                            explainableMutationCommand.getOriginalComment(), explainableMutationCommand.getMutatedComment(), "",
                            failingTest, totalTests, command.toString());
                    report.record(trial);
                }
            }
        }
     ...
     * @param version
     */
    private void record(String projectName, String version, String[] failingTests, String message,
